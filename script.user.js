...
// Track previous state for highlighting changes
var previousDict = {};
var previousLengthCountsByLetter = {};

function rebuildDictionaries() {
    // Store previous state
    previousDict = JSON.parse(JSON.stringify(dict));
    previousLengthCountsByLetter = JSON.parse(JSON.stringify(lengthCountsByLetter));
...
function highlightCell(cell) {
    ...
    document.querySelectorAll('.cell-last-updated').forEach(c => {
        c.classList.remove('cell-last-updated');
    });
    
    // Add flashing animation
    cell.classList.add('cell-flash');
    
    // After flash completes, add persistent highlight
    setTimeout(() => {
        cell.classList.remove('cell-flash');
        cell.classList.add('cell-last-updated');
...
function findChangedCells() {
    ...
    // Check two-letter pairs for changes
    for (let pair in dict) {
        const prevCount = previousDict[pair] || 0;
        const currentCount = dict[pair];
        
        if (currentCount > prevCount) {
            // Find the cell with this pair
            const cell = document.querySelector(`.cellwide[data-pair="${pair}"]`);
            if (cell) {
    ...
    // Check length counts for changes
    for (let letter in lengthCountsByLetter) {
        const prevLengthCounts = previousLengthCountsByLetter[letter] || {};
        const currentLengthCounts = lengthCountsByLetter[letter];
        
        for (let length in currentLengthCounts) {
            const prevCount = prevLengthCounts[length] || 0;
            const currentCount = currentLengthCounts[length];
            
            if (currentCount > prevCount) {
                // Find the cell with this letter and length
                const cell = document.querySelector(`.cell[data-letter="${letter}"][data-length="${length}"]`);
                if (cell) {
                    changedCells.push(cell);
                }
                
                // Also highlight the sum cell for this letter
                const sumCell = document.querySelector(`.cell[data-letter="${letter}"][data-sum="true"]`);
                if (sumCell) {
                    changedCells.push(sumCell);
                }
            }
        }
    }
...
function updateTable() {
    ...
    changedCells.forEach(cell => highlightCell(cell));
...
function continue_processing()
{
    ...
    /* Flashing animation for newly updated cells */
    ...
    .cell-flash {
        animation: flash-update 0.5s ease-in-out 4;
